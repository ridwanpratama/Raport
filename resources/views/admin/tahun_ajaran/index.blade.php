    ...
    <div class="section-body">
        <div class="row">
            <div class="col-12 col-md-12 col-lg-12">
                <a href="{{ route('tahun_ajaran.create') }}" class="btn btn-icon icon-left btn-primary"><i
                            ...
                            <tbody>
                                @foreach ($tahun_ajaran as $data)
                                    <tr>
                                        ...
                                        <td><a href="">
                                                <form action="{{ route('tahun_ajaran.destroy', [$data->id]) }}"
                                                    method="post">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button class="btn btn-danger btn-sm"
                                                        onclick="return confirm('apakah anda yakin ingin menghapus data: {{ $data->tahun_ajaran }}')">Hapus</button>
                                                    <a href="{{ route('tahun_ajaran.edit', [$data->id]) }}"
